...
export default function Page({articles, page_num}: PageProps<ArticleProps>): ReactNode {
    ...
    const deletePrompt = (id: number) => {
        Swal.fire({
            ...
            if(result.isConfirmed) {
                router.delete(route('operator.articles.delete', id))
            }
        })
    }
  ...
  return(
    ...
    <AdministratorLayout data={menus('articles')}>
      ...
      <div className="flex">
            <Link href={route('operator.articles.create')}>
            ...
            <Button className="m-2" onClick={() => {
                router.get(route('operator.articles', {
                    search
                }))
            }}>Cari</Button>
      </div>
                        ...
                        <TableCell>
                            <div className="flex space-x-4">
                                <Button className="bg-amber-500 text-white hover:bg-amber-500" asChild>
                                    <Link href={route('operator.articles.edit', row.id)}><Pencil /></Link>
                                </Button>
