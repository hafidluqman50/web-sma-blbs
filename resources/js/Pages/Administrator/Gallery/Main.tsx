...
export default function Page({galleries, page_num}: PageProps<GalleryProps>): ReactNode {
    ...
    const deletePrompt = (id: number) => {
        Swal.fire({
            ...
            if(result.isConfirmed) {
                router.delete(route('administrator.galleries.delete', id))
            }
        })
    }
  ...
  return(
    ...
    <AdministratorLayout data={menus('galleries')}>
      ...
      <div className="flex">
            <Link href={route('administrator.galleries.create')}>
            ...
            <Button className="m-2" onClick={() => {
                router.get(route('administrator.galleries', {
                    search
                }))
            }}>Cari</Button>
      </div>
                        ...
                        <TableCell>
                            <div className="flex space-x-4">
                                <Button className="bg-amber-500 text-white hover:bg-amber-500" asChild>
                                    <Link href={route('administrator.galleries.edit', row.id)}><Pencil /></Link>
                                </Button>
